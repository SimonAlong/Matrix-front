...
import styles from './CommonConfigList.less';
...
const ImportConfigForm = Form.create()(prop => {
  const { modalVisible, form, handleImportConfig, hideImportConfigModal } = prop;
  const okHandle = () => {
    form.validateFields((err, fieldsValue) => {
      if (err) return;
...
const ValueView = Form.create()(prop => {
  const { modalVisible, form, handleAdd, hideValueCodeModal, onChangeLookConfigItem, editorDidMountLookConfigItem, currentValue } = prop;
  const okHandle = () => {
...
@connect(({ commonConfigModel, authModel, profileManagerModel, loading}) => ({
  commonConfigModel,
  authModel,
  profileManagerModel,
  loading: loading.models.commonConfigModel,
}))
// @Form.create() 是一个注解，就简化了xxx = Form.create(xxx);export xxx
@Form.create()
class CommonConfigList extends PureComponent {
  state = {
  ...
  getPageData(pageNo, searchParamInput) {
    const { dispatch } = this.props;
    const {commonConfigModel: { profile }} = this.props;
    const {
      commonConfigModel: { searchParam, pager },
    } = this.props;
    ...
    // 获取页面的总个数
    dispatch({
      type: 'commonConfigModel/getPage',
      payload: {
        pager: pagerFinal,
        searchParam: param,
      },
  ...
  clock = (record) => {
    const {
      commonConfigModel: { profile },
    } = this.props;
    router.push({
      pathname: '/commonConfig/commonConfigItem',
      state: {
        appId: record.id,
  ...
  showValueCodeModal = (valueType, value) => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'commonConfigModel/setCurrentValue',
      payload: value,
    });
  };
  ...
  setTableLoading = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'commonConfigModel/setTableLoading',
    });
  };
  ...
  showAppOverViewValue =(appId) =>{
    const { dispatch } = this.props;
    const {
      commonConfigModel: { currentValue, profile },
    } = this.props;

    this.showValueCodeModal(0, "");

    dispatch({
      type: 'commonConfigModel/getAppOverview',
      payload: {
        appId,
        profile
      }
  ...
  handleImportConfig = fields => {
    ...
    dispatch({
      type: 'commonConfigModel/importConfig',
      payload: params,
    });
  ...
  onRatioChange = e => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'commonConfigModel/setProfile',
  ...
  render() {
    const {
      commonConfigModel: { selectState, groupAllCodeList, currentValue },
    ...
    const { addModalVisible, importConfigModalVisible, editModalVisible, valueViewVisible, item } = this.state;
    ...
    const {
      commonConfigModel: { totalNumber, pager, tableList, tableLoading, profile },
    } = this.props;
