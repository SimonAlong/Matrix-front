...
import styles from './CommonConfigItemList.less';
    ...
    >
      ...
      <FormItem labelCol={{ span: 3 }} wrapperCol={{ span: 20 }} label="描述">
        {form.getFieldDecorator('keyDesc', {
          ...
          rules: [{ required: true, message: '请输入keyDesc！' }],
        })(
          <TextArea
            placeholder="应用描述"
            autoSize={{ minRows: 3, maxRows: 6 }}
...
@Form.create()
class CommonConfigItemList extends PureComponent {
  state = {
  ...
  saveInputInfo = (state) => {
    ...
    // 获取数据
    this.getPageData(1, {appId, profile});
  };
  ...
  showDeleteConfirm = row => {
    const { dispatch } = this.props;
    const {configItemModel: { profile, appId}} = this.props;
    // //console.log('点击');
  ...
  getDynamicKeyValue =()=>{
    const { form } = this.props;
    const { dispatch } = this.props;
    const {configItemModel: { profile, appId, overViewKey}} = this.props;
  ...
  render() {
    const {
      configItemModel: {
        ...
        totalNumber, pager, tableList, tableLoading, configItemKeyDynamicList
      },
    ...
    return (
      <PageHeaderWrapper>
        <Divider orientation="left"><h2>应用：{appName}</h2></Divider>
...
export default CommonConfigItemList;
